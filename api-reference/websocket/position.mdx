...
- `size`, `price`, `entry_price`, `margin`, `funding_index`, `initial_margin`, `initial_margin_mark`, `maintenance_margin`, `mark_price`, `unrealized_pnl`, `liq_price` are strings to preserve precision.
...
| size | string | Signed base units (scaled) |
| price | string | Entry price alias (scaled) |
| entry_price | string | Same as `price` for clarity |
| margin | string | Margin allocated to position (quote units, scaled) |
| funding_index | string | Position funding index |
| epoch | integer | Internal epoch/version |
| is_cross | boolean | Margin mode |
| initial_margin | string or null | Required initial margin (entry notional) |
| initial_margin_mark | string or null | Required initial margin (mark notional) |
| maintenance_margin | string or null | Required maintenance margin |
...
| unrealized_pnl | string | Signed PnL (scaled) |
| liq_price | string or null | Estimated liquidation price |
