...
Choose your environment before connecting. Public feeds require no auth. Private feeds require authentication with either a bearer token or an API-key signature from the matching REST environment.

<Note>No authentication is required for the public stream. The private stream supports bearer-token auth and API-key signature auth.</Note>
...
ws.onmessage = (ev) => {
    ...
    // Application ping/pong timestamps are Unix milliseconds.
...
// Heartbeat every ~20s
setInterval(() => {
    // `ts` is optional; when provided it should be Unix milliseconds.
...
1) Obtain a bearer token via the REST Authorize endpoint or prepare an API-key signature (use the same environment as your WS connection).
2) Connect to `wss://api.ekiden.fi/ws/private` (or staging equivalent).
3) Send one authentication request once connected.
...
1) Application-level ping/pong
    ...
    - `client_ts` and `server_ts` are Unix milliseconds.
...
```json
{ "op": "pong", "server_ts": 1731541800123, "client_ts": 1731541800000, "req_id": "100004" } // both in Unix milliseconds
```
...
```
^(orderbook\.[^.]+\.[^.]+|trade\.[^.]+|ticker\.[^.]+|kline\.[^.]+\.[^.]+|all_liquidations\.[^.]+|insurance_pool\.[^.]+|adl_alert\.[^.]+)$  # public
